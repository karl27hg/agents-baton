...
- Added `notify candidates` for ready handoffs without predecessor edges and enforced project-local shift controls when successful delivery results are recorded.
- Added one controlled `notify retry` recovery record for stale, open, unclaimed work, with active-session, capacity, shift, and duplicate-recovery checks.
- Preserved the compatible notification-state projection while adding factual no-record context, latest delivery information, and recovery counts.
- Added a version-matched `baton guide show upgrade` document with current release changes, safe per-project migration steps, and consuming-project `AGENTS.md` review requirements.
