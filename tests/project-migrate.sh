...
"$CLI" --db "$TARGET_DB" migrate --check | grep 'schema=12' >/dev/null
...
"$CLI" --db "$EXPLICIT_PROJECT/.baton/baton.sqlite3" migrate --check | grep 'schema=12' >/dev/null
...
"$CLI" --db "$IN_PLACE_DB" migrate --check | grep 'schema=12' >/dev/null
