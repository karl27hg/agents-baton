...
- When a host-accepted notification is externally verified to have ended in a terminal receiver-side failure, a role with `notification.observe` may append one `notify observe` record using only the fixed result and reason-class allowlists. Do not store raw host errors or treat this audit fact as a claim, lifecycle transition, or extra recovery allowance.
...
- A completed blocking result may link multiple CRs. Require the worker to repeat `finish --outcome-cr` for every known blocker. If another blocker is discovered later, use `handoff outcome-cr-link` under `handoff.evidence_correct`; do not rewrite the original completion or infer resolution until every linked CR is implemented.
