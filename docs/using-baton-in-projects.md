...
Use `cr create-handoff` for normal implementation assignment. When an existing general handoff was completed with the exact `cr:<CR-ID>` source reference, the assigned reviewer can inspect the mechanically eligible candidate shown by `cr status` or `cr show` and adopt it with `cr link-handoff`. Candidates appear by default only while the CR is approved and has no official implementation link. `--include-related-handoffs` shows all exact-source unlinked records under the neutral `related_handoff_unlinked` label. The command rejects unfinished or blocking results, unverifiable effective commit evidence, source mismatches, and cross-CR implementation reuse. Baton records the explicit link but never infers or backfills it automatically. A blocking completed implementation also prevents `cr mark-implemented`.

RC7 keeps schema v13 and derives blocking contexts without changing completed rows. Existing `outcome.blocking` and report `blocking_outcomes` values remain cumulative compatibility totals. Use the relationship-aware categories and `handoff show` instead of interpreting those totals as active blockers. Current and historical review claimant projections are separated without rewriting the stored claimant or its audit events.
...
Schema v15 preserves all of that history, backfills each existing non-null `outcome_cr_id` as the first ordered relationship, and adds bounded notification observations. It grants the new `notification.observe` permission only to `sm` and `planning`. Migration does not infer additional outcome CRs or receiver-side results.
...
When a planning or SM agent externally verifies that a host-accepted delivery later ended in a terminal receiver-side failure, it may use `notify observe <job> --notification <id> --result <value> --reason-class <value>`. This requires `notification.observe`, accepts only the CLI's fixed allowlists, and intentionally has no raw-detail option. It appends audit metadata visible in `notify list` and `notify status`; it does not change delivery, recovery, ownership, or handoff state.

For a completed blocking result with several decision records, repeat `finish --outcome-cr <CR-ID>` for every known CR. If another blocker is discovered after completion, a role with `handoff.evidence_correct` may use `handoff outcome-cr-link <job> --cr <CR-ID> --reason <text>`. The original links remain unchanged, the first remains the legacy projection, and blocking resolution requires every linked CR to be implemented.
