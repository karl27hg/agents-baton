...
After `finish`, optionally inspect active Codex delivery candidates with `notify targets <finished-job> --role <role> --from-agent <profile>`. For an already-open handoff without a real predecessor edge, use `notify candidates <ready-job>` instead of inventing a dependency. If the target role is stopped by the applicable project-local global or role control, including shift expiry, candidate inspection returns `outside_shift`; do not send a host message. The handoff remains `open` and becomes discoverable when its role resumes.
