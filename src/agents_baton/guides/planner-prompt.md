...
- Use `cr create-handoff` for normal implementation assignment. If a general handoff with the exact `cr:<CR-ID>` source reference already finished before official linkage, the assigned reviewer may inspect and adopt it with `cr link-handoff`. Adoption requires a non-blocking result and verified effective commit evidence. Never create a synthetic no-change handoff merely to repair linkage, and never infer or backfill a link from titles or similar content.
